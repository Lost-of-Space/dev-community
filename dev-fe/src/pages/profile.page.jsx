...
const ProfilePage = () => {
  ...
  const fetchUserProfile = () => {
    axios.post(import.meta.env.VITE_SERVER_DOMAIN + "/get-profile", { username: profileId })
      ...
      .catch(err => {
        setLoading(false);
        return toast.error("Error loading posts..." + message.err);
      })
  }
          ...
          profile_username.length ?
            <section className="h-cover md:flex flex-row-reverse items-start gap-5 min-[1100px]:gap-12">
              <div className="flex flex-col max-md:items-center gap-5 min-w-[250px] md:w-[30%] md:pl-8 md:border-l border-grey md:sticky md:top-[100px] md:py-10">
                <img src={profile_img} className="w-24 h-24 bg-grey rounded-full md:w-32 md:h-32" alt="user profile image" />
                ...
                <div className="flex gap-4 mt-2">
                  {
                    profileId == username ?
                      <Link to="/settings/edit-profile" className="btn-light rounded-md">Edit</Link>
                      :
                      ""
                  }
                </div>
